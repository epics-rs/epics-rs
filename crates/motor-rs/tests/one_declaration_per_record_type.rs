...
//! wrong: `RVAL`/`RRBV`/`RMP`/`REP`/`RDIF`/`RVEL`/`LRVL` were declared
//! `DBF_INT64` where `motorRecord.dbd` says `DBF_LONG` (a CA client saw them
//! promoted to `DBR_DOUBLE` instead of `DBR_LONG`), `UEIP`/`OMSL`/`HHSV`/`LLSV`
//! were `DBF_SHORT` where the `.dbd` says `DBF_MENU` (`DBR_SHORT`, not the
//! `DBR_ENUM` with choice strings a client expects), eight declared fields
//! (`CARD`, `HSV`, `LSV`, `LSPG`, `PP`, `INIT`, `PREM`, `LOCK`) were absent
//! altogether, and two fields C does not have (`RDBL_VAL`, `LOADPOS_BLOCK` —
//! the port's internal link carriers) were declared to clients.
//!
...
use motor_rs::record::dbd_generated::{RECORD_TYPES, record_fields};
...
fn records() -> Vec<Box<dyn Record>> {
    vec![Box::new(motor_rs::record::MotorRecord::default())]
}
