...
Severity: Medium — fix — **FIXED bffbfae8** (added `Hdf5Layout::ndattribute_element_attrs()` enumerating every `<attribute source="ndattribute">` on a group or dataset with its owning element path; the writer caches the first/last frame values of the referenced NDAttributes (seeded at open, updated per frame) and flushes them at close as HDF5 attributes — `OnFileOpen`/`OnFrame` take the first value (C's duplicate close re-create fails, so the open value wins), `OnFileClose` the last; `OnFileWrite` is not materialised, matching C `is_onFileOpen`/`is_onFileClose`. Datatype follows the runtime NDAttribute value (C `typeNd2Hdf`); `Undefined` skipped. Verified by `test_ndattribute_element_attr_open_close_values` (group string + dataset numeric, both phases). **UNFIXED, blocked (residuals):** (1) dataset element-attrs on the lazily-created attribute/performance datasets — rust-hdf5 0.2.17 cannot reopen a dataset by name in write mode (`H5File::dataset` errors "cannot open a dataset by name in write mode"), so only the primary detector dataset, whose create-time handle is still live at close, gets dataset element-attrs; (2) SWMR writes only the open-time **group** set before the SWMR lock — `OnFileClose` in SWMR is impossible because HDF5 forbids creating attributes after the SWMR lock (C's close-time `H5Acreate2` fails identically), and SWMR dataset element-attrs are not addressable through `SwmrFileWriter` (datasets keyed by index, not path). Both residuals are exotic and bounded by backend API limits, not the port logic.)
