...
| ADP-8 (TimeSeries integer truncation before dividing) | fix | FIXED 3b4a2894 (standalone NDPluginTimeSeries raw-array ingestion implements the truncation; receiver-model f64 path unchanged) | — |
...
#### ADP-8: TimeSeries per-point average kept as f64; C truncates to the integer element type before dividing — FIXED (3b4a2894)
Severity: High — fix
...
FIXED: the missing-feature half is now implemented. The truncation finding had two halves: (a) the existing receiver-model `SharedTsState::accumulate` (`time_series.rs`) is fed exclusively by Stats/ROIStat/Attribute computed **f64** values — the exact analogue of C feeding a downstream NDPluginTimeSeries an `NDFloat64` per-frame stats array (`NDPluginStats.cpp:549`), so `epicsType==epicsFloat64` and C also divides in f64; that path was correct and is **unchanged**. (b) The raw-integer-array ingestion path — C's `:191` truncation — was unimplemented: `NDTimeSeriesConfigure` pointed at a detector port errored. Now `crates/ad-plugins-rs/src/time_series_plugin.rs` (`TimeSeriesProcessor`, a faithful port of `NDPluginTimeSeries.cpp`) ingests raw 1-D/2-D `[numSignalsIn, numTimes]` arrays, and `averaged_value` reproduces `(epicsType)sum/numAveraged` exactly — cast the double sum to the element type first (wrapping via an `i64` intermediate for ≤32-bit types, direct for 64-bit), then integer-divide in the element type's promoted domain. The no-receiver branch of `NDTimeSeriesConfigure` (ioc.rs) stands up this plugin via `create_plugin_runtime_multi_addr(maxSignals+1)`, serving `TS_TIME_SERIES` at `addr==signal` (NDTimeSeriesN template form, SCAN "I/O Intr" push). Fixed/Circular modes, `computeNumAverage`, the `averagingTimeActual`-scaled axis, and dtype/numSignals reallocation are all ported. Tests cover the UInt8 200×3→29 truncation, Int8 negative −29, UInt16 wrap, fixed-mode fill/stop, circular wrap/rotate, 1-D-as-single-time-point, and maxSignals capping. Residual (documented): downstream `doCallbacksGenericPointer` NDArrays (2-D at `addr==numSignals` + per-signal 1-D) are not emitted — the observable waveform/scalar records are complete.
...
#### ADP-44: NDPluginCircularBuff pre-count validation status outputs not produced — **FIXED 9f8666c6 + 55898fca**
Severity: Low — fix-low
...
**FIXED**: 9f8666c6 added the running-reject and negative-reject paths (status string + param revert) and the accept path. 55898fca closed the deferred "Pre-count too high" ceiling: `maxBuffers_` is the input NDArray queue size (`NDCircularBuffConfigure` queueSize), which the runtime already threads to the plugin as `queue_size` (ioc.rs:209) — now passed to `CircularBuffProcessor::new` as `max_buffers`. `on_param_change` checks `value > max_buffers-1` in C's exact order (running → too-high → negative → commit); rejecting before commit keeps the unbounded VecDeque pre-buffer from ever holding an out-of-range pre-count, so PRE_TRIGGER + CIRC_BUFF_STATUS match C across the board. `test_pre_count_validation` covers the too-high boundary (10 rejected, 9 accepted) plus the running/negative/accept paths.

#### ADP-45: NDPluginScatter overflow-reroute / nextClient wrap semantics not reproduced
Severity: Medium — verify — **FIXED 3df960b4** (scatter routing moved into the runtime delivery path — the only layer that sees per-consumer queue state. A persistent per-attempt cursor (`nextClient_`, `plugin_data_loop` local) walks the *enabled* senders via the new `ProcessOutput::scatter_publish`, rerouting past a full/disabled consumer and dropping only on the last attempt. `NDArraySender::publish_scatter(is_last)` mirrors C's `auxStatus`/`ignoreQueueFull` so a rerouted-away full queue does **not** count a `DroppedArrays`, only the last node's drop does (NDPluginDriver.cpp:406,433-442). `ProcessResult`/`ProcessOutput` carry a `scatter: bool` marker instead of `scatter_index: Option<usize>`; `ScatterProcessor` now only flags the frame — C keeps no per-frame index, the cursor lives entirely in `nextClient_`. Tests in `ad-core-rs` runtime: reroute-past-full, drop-on-last-counted-once, per-attempt cursor persistence across frames, disabled-consumer skip. **Residual:** the MinCallbackTime-induced reroute — C reroutes past a downstream that is *queue-free but within its MinCallbackTime window* because `driverCallback` leaves `auxStatus=asynOverflow` when it skips processing (NDPluginDriver.cpp:405) — is a distinct interaction (throttle, not queue-full) and is NOT reproduced: the Rust downstream throttle runs in the receiver's own loop after `try_send` succeeds, so a throttled-but-free consumer still receives the queued frame. Separate from the queue-full reroute family fixed here.)
Rust (pre-fix): `scatter.rs:60-68` emitted `current_index % num_outputs` (raw index when `num_outputs==0`), advancing by 1 each frame; the runtime published to `senders[index % N]` with a single `try_send` and dropped (counting) on a full queue with no reroute.
...
Impact (pre-fix): Under backpressure C reroutes the frame to the next available consumer and keeps `nextClient_` advancing; Rust delivered strictly to `index % N` regardless of queue state, so per-consumer distribution and the `DroppedArrays` count differed. With all queues free the round-robin order matched.
...
#### ADP-60: StdArrays throttle does not decrement ArrayCounter; Rust has no throttle-rollback — **FIXED 724e280c**
Severity: Low — verify
...
**Fix:** threaded a `count_frame` flag through `build_publish_batch`; `process_and_publish` sets it false when a StdArrays plugin produced output that `route_output_arrays` then throttle-dropped (`produced > 0` but `ready` empty), so ArrayCounter does not advance — matching C's net-out decrement. The StdArray waveform interrupt now fires only with the routed/served output (never the fallback input), so a throttled frame serves nothing, matching C skipping the interface callback on throttle (`NDPluginStdArrays.cpp:58`). DroppedArrays still advances (the throttle counts it). Regression: `std_arrays.rs::test_std_arrays_throttled_frame_does_not_advance_array_counter`. (The serve-only-routed-output half also closes the throttle-interaction residual of [ADP-97].)

#### ADP-97: StdArrays waveform output (STD_ARRAY_DATA) wrongly gated by NDArrayCallbacks — **FIXED 0193d658**
Severity: Medium — fix (discovered while fixing ADP-59)
Rust: `plugin/runtime.rs:725` (pre-fix) gated the `STD_ARRAY_DATA` generic-pointer interrupt — which "triggers I/O Intr on ArrayData waveform" (runtime.rs:1080) — on the `deliver` flag (`NDArrayCallbacks`). The comment there mis-cited it as C++ `doCallbacksGenericPointer`.
C: `NDPluginStdArrays.cpp:71-73` `arrayInterruptCallback` fires the typed-array waveform callbacks REGARDLESS of `NDArrayCallbacks`; only the base `NDPluginDriver::endProcessCallbacks` `doCallbacksGenericPointer` (downstream NDArray) is gated by `NDArrayCallbacks` (`NDPluginDriver.cpp:257-265,287`). NDPluginStdArrays sets `NDArrayCallbacks=0` in its constructor (`:343`) yet still serves its waveforms.
Impact: Reachable at runtime independent of ADP-59 — writing `NDArrayCallbacks=0` to any StdArrays port (the param-change handler sets the runtime field, runtime.rs:1825-1826) stopped the ArrayData waveform from updating on I/O Intr. ADP-59 (StdArrays initial `NDArrayCallbacks=0`) would have made this the default, suppressing StdArrays' primary output. **Fix:** decoupled the STD_ARRAY_DATA interrupt (now fires whenever `std_array_data_param` is present, i.e. the port is a StdArrays plugin, regardless of `deliver`) from the downstream-delivery gate (`deliver` still gates only `ProcessOutput.arrays`). Routing/throttle runs when delivering OR serving the waveform. Regression: `std_arrays.rs::test_std_arrays_serves_waveform_with_callbacks_off` (fails pre-fix, passes post-fix). **Residual:** if a user enables sort mode on a StdArrays port with `NDArrayCallbacks=0`, the waveform output is routed through the sort buffer (Rust `route_output_arrays` does throttle+sort); C's per-interface serve applies only the throttle. Default sort mode is off, so this is an obscure edge.
...
Severity: High — fix — **FIXED (standard path; SWMR degrades) via rust-hdf5 0.2.22.** The standard write path now reproduces C's N-bit packing: `DatasetBuilder::datatype` stores a reduced-precision `FixedPoint` (C `H5Tset_precision`/`H5Tset_offset`) and `FilterPipeline::nbit(&dt, d_nelmts)` builds the nbit parameter tree `[nparms, need_not_compress, d_nelmts, NBIT_ATOMIC, size, order, precision, offset]`, so the on-disk datatype carries the narrowed precision and the packed chunks are byte-readable by h5py/libhdf5 (verified: an over-range value packs to its low `precision` bits, the datatype message reports `bit_precision`). The datatype override and nbit filter are coupled and applied out of band by `nbit_packing()`; `build_pipeline` (shared with the SWMR streaming path, whose `create_streaming_dataset_chunked_compressed` hardcodes `T::hdf5_type()`) still returns None for N-bit, so **SWMR mode degrades N-bit to lossless uncompressed** rather than write an nbit filter over a full-width datatype — a residual bounded by the SWMR builder having no datatype-override hook. Default precision seeded to 8 bit (C `NDFileHDF5.cpp:2340`); restricted to integer element types (float N-bit is not an areaDetector use case) and to precision in 1..=size*8.
...
#### ADP-68: BLOSC cd_values layout differs from C's reserved-slot convention — **SIGNOFF**
Severity: Medium — verify → signoff
...
**Verdict — SIGNOFF: cds[0..3] are NOT part of the stable on-disk contract; the reader's blosc plugin re-derives them.** In the hdf5-blosc filter the four reserved slots are populated by the filter plugin's `set_local` callback at write time (format version, blosc version, typesize, blocksize) and re-derived on read — they are write-side scratch, not bytes a decoder trusts from the stored message. The decode path keys on cds[4..6] (level/shuffle/compressor) plus the chunk's own self-describing blosc header, both of which Rust authors correctly. So Rust hardcoding `[2,2,element_size,0]` in cds[0..3] is reader-tolerable: a standards-compliant blosc plugin decompresses the chunk from its header regardless of the stored reserved slots. The remaining divergence (exact reserved-slot bytes in the filter *message*) is below the output-form-parity bar — it is plugin-runtime scratch, not an observable data/format value — matching the SR-9/queueSize class of "runtime-dependent value, not a parity contract". No fix.
...
Severity: Medium — fix — **FIXED 9ef09199 (1-D scalar) + via rust-hdf5 0.2.22 (multi-dim arrays).** The standard detector-dataset path writes `NDArrayNumDims` (scalar int32, always) and `NDArrayDimOffset`/`NDArrayDimBinning`/`NDArrayDimReverse` in native dim order — a scalar int32 for a 1-D array and a 1-D int32 array of length ndims for `ndims>1` (C `writeH5attrInt32` scalar-vs-array dispatch, NDFileHDF5.cpp:1142-1191), using `AttrBuilder::shape([ndims])` + `H5Attribute::write_array`, byte-readable by h5py/libhdf5 with the correct shape and values. **Distinct gap, now FIXED 2f04c26a:** the SWMR write path previously omitted these NDArray default attributes entirely (the scalar case too); `write_swmr_ndarray_default_attrs` now writes them on the streaming dataset before `start_swmr()` via `set_dataset_attr_numeric`/`set_dataset_attr_array` (by index) — scalar for 1-D, int32[ndims] array for multi-dim, native order, matching the standard-path emission. **Deferred to ADP-61/62/63:** the default-layout `signal=1` attr is part of the NeXus default-layout feature, not this finding.
...
Severity: Medium — fix — **FIXED d5664763** (Standard non-SWMR path now builds C's `nExtraDims+1` separate **fixed** leading axes ordered `[eds[N],…,eds[1],eds[0],frameY,frameX]` (the N virtual scan dims + innermost frames-per-point "N" axis), rank `ndims+N+1`, matching `NDFileHDF5::configureDims` NDFileHDF5.cpp:3121-3230 and the doc axis order NDFileHDF5.rst:375-386. New `standard_layout` emits the multi-dim shape/chunk (leading axes chunked at 1); `flush_band` places each frame at its `unravel()` odometer position via `write_chunk_at` multi-dim coords — frame bytes are row-major identical to the old collapsed form (C `extendDataSet` increments innermost-first), so only the dataspace rank changed. `set_n_extra_dims` clamped to `MAX_EXTRA_DIMS-1` so the outermost `extra_dims[N]` is in bounds (C rejects `nExtraDims>9`, NDFileHDF5.cpp:1789). Tests `test_extra_dimensions_layout` (rank-5 `[4,3,2,4,4]`) + `test_extra_dimensions_one_virtual` (rank-4 `[3,2,4,4]`). **SWMR uncompressed FIXED c4d877d7** (rust-hdf5 0.2.23 added `create_grid_dataset` (fixed multi-dim leading shape) + positioned `write_chunk_at`; `open_swmr` now builds the same `standard_layout` grid `[eds[N],…,eds[0],Y,X]` when `extra_dim_axes()` is Some and uncompressed, and `write_swmr` places each frame at its `unravel()` odometer position. The standard `flush_band` and the SWMR grid path share one `band_chunk_writes` tile-math helper. `close_file` flushes before close so the grid's fixed-array index is committed. Tests `test_swmr_extra_dimensions_grid_layout` (rank-5), `_grid_subtiled` (row tiling), `_grid_partial_fill` (fill tail). **Residual (compressed only):** a multi-extra-dim dataset that ALSO requests HDF5 compression stays collapsed — there is no compressed grid constructor, and rust-hdf5 rejects a filter pipeline on a v2-B-tree dataset (dataset.rs:242); the uncompressed grid uses a fixed-array index, not v2 B-tree, so it is unaffected.)
...
Impact: For `HDF5_nExtraDims=N`, C writes a rank `ndims+N+1` dataset with separate unlimited extra-dim axes; Rust collapses all extra dims into ONE leading axis of size `product(sizes)`, rank `1+ndims`, recording the intended shape only as `HDF5_extraDimSize*` attrs. A reader sees a different rank/shape with extra dims configured. The single-extra-frame-dim case (the common one) already matches; the divergence is only `HDF5_nExtraDims>=2`.
Backend feasibility (2026-06-16): **NOT backend-blocked.** rust-hdf5 0.2.17 supports two-or-more unlimited dimensions via `DatasetBuilder::max_shape(&[None, None, …])` (v2 B-tree chunk index, dataset.rs:226-242) with chunk writes addressed by grid coordinates through `Dataset::write_chunk_at(&[coords…], bytes)` (dataset.rs:746-766, doc example `.shape(&[0,0]).chunk(&[2,2]).max_shape(&[None,None])`). Caveat: rust-hdf5 rejects a **filter pipeline** on a v2-B-tree dataset ("compression of v2 B-tree (multi-unlimited-dimension)", dataset.rs:242) — so a multi-extra-dim dataset that ALSO requests HDF5 compression is still backend-blocked; the uncompressed multi-extra-dim case is implementable. Implementation is substantial: rework `primary_layout` to emit `nExtraDims+1` unlimited leading axes and route streaming writes through `write_chunk_at` with multi-dim chunk coordinates instead of the current single linear `write_chunk` frame index. Contained to `file_hdf5.rs` but touches the core dataspace/streaming-write path shared by the common single-axis case → regression risk; gate on sign-off.
...
Severity: Medium — fix — **FIXED bffbfae8** (added `Hdf5Layout::ndattribute_element_attrs()` enumerating every `<attribute source="ndattribute">` on a group or dataset with its owning element path; the writer caches the first/last frame values of the referenced NDAttributes (seeded at open, updated per frame) and flushes them at close as HDF5 attributes — `OnFileOpen`/`OnFrame` take the first value (C's duplicate close re-create fails, so the open value wins), `OnFileClose` the last; `OnFileWrite` is not materialised, matching C `is_onFileOpen`/`is_onFileClose`. Datatype follows the runtime NDAttribute value (C `typeNd2Hdf`); `Undefined` skipped. Verified by `test_ndattribute_element_attr_open_close_values` (group string + dataset numeric, both phases). **Residuals (1) + (2a) FIXED via rust-hdf5 0.2.22; (2b) remains by HDF5 design:** (1) dataset element-attrs on the lazily-created attribute/performance datasets are now attached by reopening the dataset by full path in write mode (`H5File::dataset_writer`), so every dataset — not just the detector dataset whose create-time handle is live at close — gets its element-attrs (`test_ndattr_element_attr_on_lazily_created_dataset`); (2a) open-time (`OnFileOpen`/`OnFrame`) dataset element-attrs on the SWMR streaming dataset are attached before the SWMR lock via `set_dataset_attr_*` (datasets now addressable by index from a path) (`test_swmr_ndattr_element_attr_on_streaming_dataset`); (2b) **`OnFileClose` in SWMR remains impossible** — HDF5 forbids creating attributes after the SWMR lock and C's close-time `H5Acreate2` fails identically, so this is a genuine backend-design limit matching C, not a port gap.
...
#### ADP-80: `<global name="detector_data_destination">` parsed but never used to route the detector dataset — **FIXED df047ea7**
Severity: Low — verify
...
**FIXED df047ea7**: confirmed real and reachable — a layout with multiple `<dataset source="detector">` nodes wrote a file missing every non-default dataset, and the destination attribute had no effect. The standard-mode writer now creates every detector dataset up front (`Hdf5Handle::Standard.detectors` map keyed by C full name, from `Hdf5Layout::detector_dataset_paths`), each with its own handle/frame-count/band, and routes each frame by `resolve_destination_key` (default `det_default`; the named key when the routing NDAttribute is a string matching an existing dataset; default on absent/unknown; error on present-but-non-string, mirroring C `getValue(NDAttrString)`→`ND_ERROR`). `finalize_standard_datasets` flushes and trims each dataset to its own count. Tests `test_detector_data_destination_routes_by_attribute` (per-dataset extents + content for default/named/unknown-fallback) and `test_detector_data_destination_non_string_attribute_errors`. Residual: SWMR multi-detector routing stays single-dataset (the `SwmrFileWriter` backend streams one dataset), as with ADP-74's SWMR collapse.
...
### compression-aware gate — NDPluginCodec / NDFileHDF5 (ADP-98..99)

These two findings surfaced while signing off [ADP-57]. The shared anchor is the runtime drop gate `if compressed && !guard.compression_aware` (`runtime.rs:1785`), the structural mirror of C `NDPluginDriver::driverCallback` `if (!compressionAware_ && !pArray->codec.empty()) { droppedArrays++; return; }` (`NDPluginDriver.cpp:383-393`). A plugin that C marks `compressionAware=true` (so compressed arrays reach it) MUST override `NDPluginProcess::compression_aware()` in Rust, or the runtime drops its compressed input before the plugin runs. Workspace audit (`rg "fn compression_aware" crates/`) finds ZERO overrides — only the default `false` at `runtime.rs:256`. C audit of every plugin's base-constructor `compressionAware` argument: only **NDPluginCodec** (`NDPluginCodec.cpp:870` passes `true`) and **NDFileHDF5** (`NDFileHDF5.cpp:2268` passes `true`) are compression-aware; all other plugins (StdArrays, Attribute, JPEG/Magick/Nexus/TIFF/NetCDF/Null file writers, every math plugin) pass/inherit `false` and match the Rust defaults.

#### ADP-98: CodecProcessor missing compression_aware() override — Decompress mode is dead at the runtime gate — **FIXED 9003e977**
Severity: High — fix
Rust: `codec.rs:932` `impl NDPluginProcess for CodecProcessor` does not override `compression_aware()`, so it inherits the default `false` (`runtime.rs:256`). The runtime drop gate (`runtime.rs:1785-1789`) then drops any array with `codec.is_some()` and increments `DroppedArrays` BEFORE `process_array` runs. `CodecProcessor`'s entire `CodecMode::Decompress` path (`codec.rs:966-986`, which reads `array.codec` and dispatches to the LZ4/zlib/bslz4/JPEG/blosc decoders) therefore never receives a compressed array — it is dead code at the runtime level.
C: `NDPluginCodec` passes `compressionAware=true` to the base constructor (`NDPluginCodec.cpp:865-870`), unconditionally regardless of mode, precisely so compressed arrays reach it for decompression (`NDPluginCodec.cpp` `decompress*` paths).
Impact: Observable, reachable divergence. `NDCodecConfigure` is wired into the IOC (`ioc.rs:224`) and its mode is switchable to `Decompress` at runtime (`codec.rs:1081`). A detector/upstream producing compressed NDArrays (e.g. Eiger LZ4/bslz4) feeding a downstream Codec-in-Decompress: C emits the decompressed downstream arrays; Rust drops every one (DroppedArrays climbs, zero output). `compression_aware()` is read once at construction (`runtime.rs:1564/2096`), so the override must return `true` unconditionally — the same instance can switch Compress↔Decompress at runtime, matching C's unconditional `true`. Compress mode is unaffected (its input is uncompressed, `codec.is_none()`).

#### ADP-99: NDFileHDF5 pre-compressed direct-chunk-write not ported; HDF5 writer not compression-aware — **FIXED 78408f29** (depends on [ADP-100])
Severity: Medium — feature — **FIXED** (rust-hdf5 0.2.19 added `write_chunk_raw`, unblocking the backend gap below)
Rust: `file_hdf5.rs` `Hdf5FileProcessor` does not override `compression_aware()` (inherits `false`), AND has no path to consume a pre-compressed input array: its compression machinery (`build_pipeline`, `compression_type`, deflate/blosc, `file_hdf5.rs:574-580`) applies the writer's OWN filter to RAW input on write; it never reads `array.codec` to write already-compressed bytes through. So merely overriding `compression_aware()->true` here would forward a compressed array the writer treats as raw pixels → garbage on disk. This is a feature gap, not a one-line override.
C: `NDFileHDF5` passes `compressionAware=true` (`NDFileHDF5.cpp:2268`) and implements direct-chunk-write: `configureCompression` reads `pArray->codec` and overrides the HDF5 filter settings from the pre-compressed array (`NDFileHDF5.cpp:3314-3331` — BLOSC/BSLZ4/LZ4/JPEG → matching `HDF5Compress*` type), so the compressed bytes are written through the corresponding HDF5 filter without re-compression (libhdf5 `H5DOwrite_chunk` with a `filters` mask).
Impact: With a pre-compressed input array, C writes a chunked dataset carrying the compressed bytes under the matching HDF5 filter (the areaDetector direct-chunk-write optimisation); the Rust writer cannot reproduce this. **Backend verification (2026-06-16):** rust-hdf5 0.2.17 is a pure-Rust HDF5 implementation; its only chunk-write entry points are `Dataset::write_chunk`/`write_chunk_at` (dataset.rs:667/766), both of which delegate to `io::writer.rs::write_chunk`, which **unconditionally** runs `filter::apply_filters(pipeline, data)` on the raw bytes (writer.rs:1408-1416) and hardcodes `filter_mask: 0` in the recorded `FilteredChunkEntry` (writer.rs:1449). There is **no** API to write already-compressed bytes through verbatim with a non-zero filter mask (no `H5DOwrite_chunk` analog). Therefore byte-parity with C's direct-chunk-write is **unachievable without an upstream rust-hdf5 change** (a `write_chunk_raw(chunk_idx, compressed_bytes, filter_mask)` that skips `apply_filters` and records the mask). That is a separate-repo feature (user maintains rust-hdf5 at `~/codes/rust-hdf5`), out of `epics-rs` scope, and needs sign-off. Until then the `compression_aware()` override must NOT be applied to the Rust HDF5 writer standalone, because the consume path does not exist. This is the HDF5 sibling of [ADP-98]; ADP-98 (Codec plugin) is the closable half and is FIXED.
**Fix (78408f29, 2026-06-16):** rust-hdf5 0.2.19 added `Dataset::write_chunk_raw(chunk_idx, bytes, filter_mask)` (skips `apply_filters`, records the mask) — the `H5DOwrite_chunk` analog the backend lacked. `Hdf5FileProcessor::compression_aware()` now returns `true`; a codec-bearing frame builds the dataset with the codec's original element type + a `codec_filter_pipeline` derived from `array.codec`, and `codec_chunk_bytes` assembles the on-disk chunk per `NDFileHDF5Dataset::writeFile` (LZ4 16-byte header; BSLZ4 12-byte header = u64 uncompressed bytes + u32 block bytes; BLOSC/JPEG verbatim) before `write_chunk_raw(idx, cb, 0)`. Mid-stream codec change and SWMR-with-codec are rejected. LZ4/BLOSC/BSLZ4 all round-trip through rust-hdf5's reverse filters in tests (BSLZ4 once rust-hdf5 0.2.21 made its bitshuffle filter canonical, [ADP-101]); canonical byte parity is additionally locked by [ADP-100]'s C-reference-vector test.

#### ADP-100: bslz4 codec emitted a non-canonical bit-transpose layout — **FIXED e78cfa97**
Severity: High — output-form (codec byte format) — **FIXED**
Rust (before): `codec.rs` `compress_bslz4`/`decompress_bslz4` used a hand-rolled bit transpose that round-tripped through its own inverse but did NOT match the bitshuffle library. Verified by compiling `hdf5_plugins/BSHUF/src/bitshuffle_core.c` (scalar path) and running `bshuf_bitshuffle` on the u16 ramp 0..15: the library emits `[170,170,204,204,240,240,0,255,0…]` (LSB-first `TRANS_BIT_8X8`, elem k → output bit k); the old port produced a different permutation.
C: H5Zbshuf / NDPluginCodec call the canonical bitshuffle library (`bshuf_compress_lz4` → `bshuf_blocked_wrap_fun`), so every `bslz4` byte areaDetector/h5py emit follows that layout.
Impact: every `compress_bslz4` consumer (NDPluginCodec runtime, and the file/PVA carriers downstream) produced `bslz4` bytes that h5py/libhdf5/C could not decode. Now byte-for-byte canonical: scalar byte/bit/bitrow transposes + their exact inverses, blocked framing (full blocks → one partial block rounded down to mult of 8 → final `size%8` elements raw), per-block `[u32 nbytes_BE][lz4]`, no chunk-level header in the codec buffer (that belongs to the HDF5 chunk). Locked by `codec::tests::test_bitshuffle_matches_c_reference_vector`.

#### ADP-101: rust-hdf5's bitshuffle HDF5 filter is non-canonical (MSB-first) — compress-on-write BSHUF files are not h5py/C-readable — **FIXED ca6a038a** (rust-hdf5 0.2.21)
Severity: Medium — output-form (on-disk filter bytes) — **FIXED** (rust-hdf5 0.2.21 made the filter canonical)
Rust: `rust-hdf5/src/format/messages/filter.rs` `bitshuffle_block`/`bitunshuffle_block` number bits MSB-first (`bit_idx = 7 - (bit % 8)`) and reverse element order within each 8-group. For the u16 ramp 0..15 this yields `[0,0,0,0,0,0,0,0,0,255,15,15,51,51,85,85,…]` — a bit-reversed permutation of the canonical `[170,170,204,204,240,240,0,255,…]`. The forward and inverse are mutually consistent, so rust-hdf5 round-trips its OWN bitshuffle files, but it cannot read libhdf5/h5py/C bitshuffle data and h5py/C cannot read what it writes.
C: H5Zbshuf uses the canonical bitshuffle library (LSB-first `TRANS_BIT_8X8`), so C-written bitshuffle datasets are h5py/libhdf5-interoperable.
Impact (reachable): two consequences. (1) The compress-on-write path `Hdf5Writer::set_compression_type(COMPRESS_BSHUF)` routes RAW data through rust-hdf5's filter pipeline (`build_pipeline` → `FilterPipeline` bitshuffle), so the resulting `.h5` carries non-canonical bytes that h5py/C cannot decode — a parity divergence vs C. (`test_bitshuffle_compressed_write` passes only because rust-hdf5 reads back its own non-canonical bytes.) (2) ADP-99's BSLZ4 direct-chunk-write stores CORRECT canonical bytes ([ADP-100]) but rust-hdf5's own reader decodes them to garbage, so the in-crate round-trip test cannot verify pixels (asserts structure + header instead).
**Fix (ca6a038a, 2026-06-16):** the root cause was fixed in rust-hdf5 0.2.21 (`bitshuffle_block`/`bitunshuffle_block` now number bits LSB-first, matching `TRANS_BIT_8X8`; the same u16 ramp 0..15 now yields the canonical `[170,170,204,204,240,240,0,255,…]`). ad-plugins bumped `0.2.19 → 0.2.21`. Both consequences close: compress-on-write BSHUF emits canonical bytes, and `test_direct_chunk_write_bslz4_roundtrips` reads this crate's canonical BSLZ4 direct-chunk-write bytes back to the original pixels (restored from structure+header to a full pixel round-trip). Encoder byte format stays locked by [ADP-100]'s C-reference-vector test.
