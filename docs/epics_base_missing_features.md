...
- **서버 측 채널 필터 (Server-side Filters, 3.15.7)** — 🔄 **PARTIAL**: Stages 1-5 — Framework + **5개 필터** (`dbnd`, `arr`, `ts`, `dec`, **`sync`**) + JSON 파서 + **CA 서버 wire-through**. `CA_PROTO_CREATE_CHAN`에서 `split_channel_name`으로 record path/JSON suffix 분리, `ChannelEntry.filter_suffix`에 stash. `CA_PROTO_EVENT_ADD`가 `parse_filter_chain`으로 chain 구성 → subscriber attach. **신규 sync 필터** (`crates/epics-base-rs/src/server/database/filters/sync.rs`): JSON `{"sync":{"trigger":"PV"}}` 구문, "after" 모드 — trigger PV가 process 할 때 `SyncRegistry::fire(name)`로 모든 listening gate를 arm, 다음 value 이벤트가 arm을 소비. Alarm/property 이벤트는 446e0d4a 규칙으로 arm 소비 없이 통과. 프로세스 전역 `SyncRegistry::registry()` (OnceLock)가 trigger 이름→gate 매핑 보유, 드롭된 filter는 fire 시 lazy reap. 7 단위 테스트 (disarmed drop / arm consume / alarm 통과 / registry multi-gate fire / dropped reap / 파서 양·음 케이스). **남은 작업**: PVA 서버 wire-through (pvRequest의 `record_options`/`field()` 구문이 CA의 JSON suffix와 다름 — 별도 architectural bridge 필요) + 5종 추가 sync 모드(`before`/`first`/`last`/`unless`/`while`) + trigger PV process 경로에서 `SyncRegistry::fire(name)` 자동 호출.
