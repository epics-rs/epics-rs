...
- Event-scan routing: `eventNameToHandle` whitespace trim, `[1,255]` numeric canonicalisation, event `0` → no event, and per-`EVNT` list membership (`scan_index.rs:148-198` vs `dbScan.c:469-552`). The `postEvent` iocsh command was NOT clean and this entry claimed it was: C declares one `iocshArgString` "event name" and calls `postEvent(eventNameToHandle(sval))` (`dbIocRegister.c:471-480`), while the port declared an integer argument — so every named `EVNT` was refused by the argument parser before the handler ran — and printed a `Posted event` line C never prints. Fixed in `ad701f02`.
...
- macLib port: `$` + `(`/`{` macro-reference detection, no expansion inside single quotes, `$(name=default)` and `$(name,k=v,…)` scoped-macro push/pop, self-reference recursion guard (`db_loader/mod.rs:511-740` vs `macLib/macCore.c:688-860`). The `\`-escape passthrough this entry recorded held only for the caller's own string: `discard = (level > 0)` (`macCore.c:700-703`) was absent, so a quote or a backslash that reached the expander through a macro value, a reference name or a default survived where C removes it. Boundary table now in `quotes_and_escapes_survive_only_at_level_zero`.
