...
|---|---|---|---|
| **mio 1.2** (under tokio) | blocked (not by design) | no epoll/kqueue on RTEMS. mio *does* ship `selector/poll.rs`, but its `cfg` list (`src/sys/unix/mod.rs:41-54`) names aix/espidf/fuchsia/haiku/hermit/hurd/nto/solaris/vita/cygwin/wasi and not rtems. Adding an `rtems` arm is not the cheap win it looks: those targets pair with `waker/pipe.rs`, and RTEMS `pipe()` returns an IMFS FIFO that rtems-libbsd `poll()` fails immediately with POLLERR (measured, `doc/upstream-rtems-bugs/evidence/FINDING-1-libevent-poll-spin.md`) — a socketpair waker would be needed first. `kqueue` avoids the family via `EVFILT_USER` but has no libc bindings yet | §4 — not needed today: the single RTEMS backend is blocking thread-per-client for both CA and PVA, so no selector is ever constructed |
