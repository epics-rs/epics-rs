...
- **서버 측 채널 필터 (Server-side Filters, 3.15.7)** — 🔄 **PARTIAL**: Stages 1+2 완료 — Framework (`SubscriptionFilter` trait, `FilterChain`, `FilteredMonitorEvent`) + **4개 표준 필터 구현 완료**: `dbnd`(deadband), `arr`(array slice, neg-idx + stride), `ts`(timestamp rewrite), `dec`(decimate every Nth). 3개 subscriber notify 경로(`notify_field_with_origin`, `notify_from_snapshot`, `ProcessVariable::{notify_subscribers, post_alarm}`)가 chain을 호출. 446e0d4a "alarm/property always pass" 시맨틱은 dbnd/arr/dec 모두 적용. `attach_filter_to_last_subscriber` API로 chain 구성 가능. 33 tests(unit 31 + integration 2). 남은 작업: PV-name JSON 파서 (`PV.{"dbnd":{"d":0.5}}`) + `sync` 필터 (외부 trigger PV 종속) + CA/PVA 채널 생성 경로 wire-through.
