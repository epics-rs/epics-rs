...
#### PS-52 `fork_and_go` child branch `?`-aborts the daemon on dup2/setsid failure where C ignores both — CLEARED
- Severity: CONCERN
- Found: round-7 caucus review (rev-ps-process), final family-closure sweep of the daemon-binary startup path (the last `?`-abort in the daemon child diverging from C)
- Rust: `src/procserv/daemon.rs:130-131` (`dup2(null_fd, fd).map_err(...)?` ×3 over `[0,1,2]`) and `:133` (`setsid().map_err(...)?`) — both in the post-fork child branch of `fork_and_go`
- C: `procServ.cc:906` `ignore_result( dup(fh) )` ×3 and `:910` `setsid();` — both **unchecked**; the child branch never aborts (only the pre-fork `open` at `:877` and `fork` at `:882` `exit`, which Rust's `:79`/`:84` correctly mirror with `?`)
- Impact: After `fork()`, the parent branch unconditionally writes the pidfile with the child's pid and `exit(0)` (`daemon.rs:102-115`), independent of the child. In the child branch a `dup2` or `setsid` failure returned `Err` → the binary `eprintln!("daemonize failed")` + `ExitCode::FAILURE` → the daemon child died. Because the parent already published the pidfile and returned 0, a `Type=forking` systemd unit marks the service *active (running)* with no process — the same headless-daemon false-success the PS-48/49/50/51 family eliminates, and the same shape as PS-50 (Rust aborts on a setup op C leaves unchecked). Reachability is near-nil (`dup2` of a just-opened valid fd; `setsid` in a fresh non-leader child essentially always succeeds), hence CONCERN, not DEFECT — but it was the last `?`-abort in the daemon child that diverged from C.
- Resolution: warn-continue, matching C's `ignore_result(dup(...))` / unchecked `setsid()`. Each `dup2` failure and the `setsid` failure now `eprintln!` and proceed rather than `?`-abort; the child branch always returns `Ok(())`. This is C-faithful and harmless: the IOC's stdio comes from the PTY, not the daemon's 0/1/2, and `setsid` failing to detach from the controlling terminal does not break the daemon. `fork_and_go` keeps its `ProcServResult<()>` signature — the **pre-fork** `open(/dev/null)` (`:79`) and `fork()` (`:84`) still fail-fast with `?` (C `exit`s on them too), so the foreground process still reports those. *Intentional-divergence note:* C silently discards these failures (`ignore_result`); Rust logs and continues — observable and strictly better than both C's silence and the prior abort (steering: don't copy C's bugs). Not auto-tested: forking from the test harness is hostile (`daemon.rs` tests module says so) and the failures can't be forced deterministically; closed by inspection like PS-32/PS-50. With PS-52 closed, the daemon-child path matches C's "never abort on best-effort setup" everywhere; the only remaining post-fork abort is the tokio runtime build (`procserv_rs.rs:583`), an irreducible Rust-architecture residual with no C analog, no degraded mode, and no pre-fork alternative (runtime workers don't survive `fork()`) — accepted, not a finding. **Post-fork startup family CONVERGED.**
