...
Severity: Medium — fix — **FIXED 9ef09199 (1-D scalar) + via rust-hdf5 0.2.22 (multi-dim arrays).** The standard detector-dataset path writes `NDArrayNumDims` (scalar int32, always) and `NDArrayDimOffset`/`NDArrayDimBinning`/`NDArrayDimReverse` in native dim order — a scalar int32 for a 1-D array and a 1-D int32 array of length ndims for `ndims>1` (C `writeH5attrInt32` scalar-vs-array dispatch, NDFileHDF5.cpp:1142-1191), using `AttrBuilder::shape([ndims])` + `H5Attribute::write_array`, byte-readable by h5py/libhdf5 with the correct shape and values. **Distinct, out of scope:** the SWMR write path omits these NDArray default attributes entirely (the scalar case too) — a separate gap from this scalar-only-array residual. **Deferred to ADP-61/62/63:** the default-layout `signal=1` attr is part of the NeXus default-layout feature, not this finding.
