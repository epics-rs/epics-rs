...
//! wrong: 42 fields `tableRecord.dbd` declares
//! `special(SPC_NOMOD)` were declared writable (a `caput` to a readback like
//! `M0XL` was accepted), `MMAP` was `DBF_LONG` where the `.dbd` says
//! `DBF_ULONG`, and the eight `cvt_dbaddr` array fields (`A`, `B`, `PP0..PP2`,
//! `PPO0..PPO2`) were not declared at all.
//!
...
use optics_rs::records::dbd_generated::{RECORD_TYPES, record_fields};
...
fn records() -> Vec<Box<dyn Record>> {
    vec![Box::new(optics_rs::records::table::TableRecord::default())]
}
