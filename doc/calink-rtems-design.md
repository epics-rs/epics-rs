...
6. Everything in §10.10 that C6 did not touch remains open — in particular
   item 2, the hosted `rtems-exec-model` build's inability to exercise either
   link resolver's client path (`AsyncUdpV4` needs a reactor). That gap is
   precisely why all four defects in this section had to be found by booting
   the target.
