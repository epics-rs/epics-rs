...
static int in_calloc, in_realloc;
...
void *__wrap_malloc(size_t n)
{
    ...
    if (in_calloc) __atomic_fetch_add(&malloc_in_calloc, 1, __ATOMIC_RELAXED);
    if (in_realloc) __atomic_fetch_add(&malloc_in_realloc, 1, __ATOMIC_RELAXED);
    p = __real_malloc(n);
    blk_insert(p, n, SITE());
    return p;
}
