...
#            check gate this script LINKS, so the prefix's cross compiler
#            (`arm-rtems6-gcc` / `arm-rtems7-gcc`) must be on PATH — source
#            the prefix's `epics-rs-env.sh` (`scripts/rtems-bsp.sh`) — and
#            the bootable-image cfg `rtems_boot_linked` is set
...
case "$OS" in
    rtems)
        TOOLCHAIN=nightly
        # The linker is the prefix's series' driver (arm-rtems6-gcc or
        # arm-rtems7-gcc, read off the tree by rtems-tool-target.sh).
        # `.cargo/config.toml` names arm-rtems6-gcc for the stock triple; the
        # environment overrides it, and the JSON spec's stem has no config
        # entry at all.
        _linker="$(./scripts/rtems-tool-target.sh)-gcc"
        export CARGO_TARGET_ARMV7_RTEMS_EABIHF_LINKER="$_linker"
