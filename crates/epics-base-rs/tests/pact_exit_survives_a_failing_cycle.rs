...
//! every path that ends a cycle, and `dbNotifyCompletion` (`dbNotify.c:468-470`)
...
//! of scope. `PactExitGuard` is what closes it, and it closes all three at once
//! without an edit at any of them — which is the point of putting the debt in a
//! `Drop` rather than at each site.
//!
...
async fn a_put_queued_behind_a_failing_cycle_still_gets_its_restart() {
    ...
    // Releasing the park arms the FIRST restart. Its replay then runs `boom`,
    // whose `Err` leaves `process_record_with_links_body` at
    // `run_registered_subroutine()?` — above every explicit release site.
...
/// harness — this one reaches the tail by the ordinary route and passes with
/// the guard defused.
#[epics_macros_rs::epics_test]
