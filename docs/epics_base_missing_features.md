...
- **IPv6 지원 (PR #205)** — 🔄 **PARTIAL (Stage 1)**: PVA TCP 서버/클라이언트 IPv6 round-trip 완성. `PvaServerConfig::bind_ip: Ipv4Addr` → `IpAddr` 일반화(`crates/epics-pva-rs/src/server_native/runtime.rs:35`), `[::1]:5075`/`[::]:5075` bind 가능. `PvaServer::client_config()`는 서버의 bind_ip family를 따라 v4/v6 loopback 자동 선택. PVA wire는 이미 16-byte IPv4-mapped IPv6 인코딩(`proto/ip.rs::ip_to_bytes`)이라 wire-level 변경 없음. pva-gateway-rs/dual-gateway-rs 두 binary도 `--bind ::1` 등 v6 주소 수용. Linux 기본 `IPV6_V6ONLY=0`으로 자동 dual-stack, BSD/macOS는 별도 v4 인스턴스 필요(주석 명시). 2 integration tests (v6 bind + dial, v6 end-to-end pvget round-trip with NTScalar<double>). **남은 작업** (Stages 2+): UDP responder/beacon emitter의 IPv6 multicast (FF0E::400), PVA client의 IPv6 SEARCH (UDP), `EPICS_PVA_ADDR_LIST` IPv6 표기 파싱. CA는 wire format 제약으로 IPv4 유지(upstream PR #205도 EPICS_HAS_IPV6=1 컴파일 플래그 DRAFT 상태).
