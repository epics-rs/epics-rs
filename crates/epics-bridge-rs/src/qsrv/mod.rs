...
pub use pva_adapter::{
    ...
    take_registered_pva_pvs,
};
// The host dual-protocol runner: gated with its definition, for the reason
// stated there (`epics_ca_rs::server::CaServer` / `epics_pva_rs::server::
// PvaServer` are both RTEMS-gated in their own crates).
#[cfg(all(feature = "qsrv-core", not(target_os = "rtems")))]
