...
| CI (`.github/workflows/rust.yml`, `rtems-closure` job, 2026-07-24) | runs `./scripts/rtems-check.sh` on a floating `nightly` + `rust-src`, so it takes the spec the same way the script always has (generated + `-Zjson-target-spec`). The float is deliberate: the generator's refusal-on-upstream-flip is the retirement trip-wire, and CI is now a place it fires. (Historical note: this row used to be "nothing to wire" because `rtems-check.sh` was red on stock `libc` and excluded from CI; the workspace `[patch.crates-io]` libc pin removed that exclusion reason.) | yes |
