...
### R5-1b (MED, OPEN): `sub`/`aSub` subroutine return-status not mapped to VAL/SEVR
`SubroutineFn = Box<dyn Fn(&mut dyn Record) -> CaResult<()>>` carries no return-status channel. C `subRecord.c::do_sub` sets `SOFT_ALARM` when the subroutine returns < 0 (`subRecord.c` `do_sub`), and `aSubRecord.c` writes the subroutine's `long` return into `VAL` (`aSubRecord.c:223` `prec->val = status`). Now that the subroutine runs (R5-1), this divergence is reachable: a sub subroutine signalling an error raises no SOFT_ALARM, and an aSub's VAL is whatever the closure wrote, not the return code. Fix needs the `SubroutineFn` signature to return a status (`-> CaResult<i64>` or similar) and the framework to apply it. Separate finding/commit.

### R5-1c (MED, OPEN): `sub` limit-alarm fields (HIHI/HIGH/LOLO/LOW/HYST/MDEL/ADEL) absent
C `subRecord.c::checkAlarmsAndMonitors` runs `recGblCheckAlarms` against HIHI/HIGH/LOLO/LOW (with HYST) and posts VAL via MDEL/ADEL deadbands. `SubRecord` (`sub_record.rs:50-57`) carries only VAL/SNAM/INP/A — no limit or deadband fields — so a sub never enters HIHI/HIGH/LOLO/LOW alarm and always posts VAL (no MDEL/ADEL gating). Reachable now that the subroutine computes VAL. Separate finding/commit.
