...
#### ADP-101: rust-hdf5's bitshuffle HDF5 filter is non-canonical (MSB-first) — compress-on-write BSHUF files are not h5py/C-readable
Severity: Medium — output-form (on-disk filter bytes) — **UNFIXED, external (rust-hdf5)**
...
Structural fix (proposed, needs sign-off): route the COMPRESS_BSHUF compress-on-write path through the canonical `compress_bslz4` codec + ADP-99 `write_chunk_raw` (bypassing rust-hdf5's non-canonical filter), so ad-plugins emits canonical bitshuffle in BOTH the pre-compressed and compress-on-write paths. Alternative: fix rust-hdf5's `bitshuffle_block`/`bitunshuffle_block` to the canonical LSB-first order (separate repo at `~/codes/rust-hdf5`, version bump + republish). The non-canonical reader would then also need fixing for in-crate round-trip tests. Either path is out of the current task's scope and is recorded here for decision.
