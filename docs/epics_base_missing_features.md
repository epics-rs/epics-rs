...
- **IPv6 지원 (PR #205)** — 🔄 **PARTIAL (Stage 1)**: PVA 서버 TCP listener가 IPv6 bind 지원. `PvaServerConfig::bind_ip: Ipv4Addr` → `IpAddr`로 일반화(`crates/epics-pva-rs/src/server_native/runtime.rs:35`), `[::1]:5075`/`[::]:5075` bind 가능. Linux 기본 `IPV6_V6ONLY=0`으로 자동 dual-stack, BSD/macOS는 별도 v4 인스턴스 필요(주석 명시). pvxs PVA wire는 이미 16-byte IPv4-mapped IPv6 인코딩(`proto/ip.rs::ip_to_bytes`). PVA gateway/dual gateway 두 binary도 `--bind ::1` 등 v6 주소 수용. 1 integration test (v6 loopback bind + dial). **남은 작업** (Stages 2+): UDP responder/beacon emitter의 IPv6 multicast (FF0E::400), PVA client의 IPv6 SEARCH, PVA client `ConnectionPool`의 IPv6 connect, CA는 wire format 제약으로 IPv4 유지(upstream PR #205도 EPICS_HAS_IPV6=1 컴파일 플래그 DRAFT 상태). upstream commits 4,000+ LOC 중 핵심 wire/encode/decode는 epics-pva-rs가 이미 IPv6-ready라 Rust 측 잔여 작업은 client side + UDP responder + 일부 환경변수 파싱(`EPICS_PVA_ADDR_LIST` IPv6 표기 지원).
...
- **`compress` 레코드 개선 (7.0.8)** — 🔄 **PARTIAL**: `PBUF` 필드 추가 + `get_field("VAL")` 분기 (`crates/epics-base-rs/src/server/records/compress.rs`). PBUF=YES 시 `nuse < nsam` 동안 valid prefix만 노출 (`val[..nuse]`), PBUF=NO/full 시 historic 전체 NSAM 벡터 노출. menu string("YES"/"NO") put도 지원. 4 unit tests. **남은 작업**: N-to-1 algorithm 내부의 partial-buffer 압축 분기 (현재는 accum.len() >= n 일 때만 emit) — push_value가 array-input 모델로 확장될 때 적용.
