...
- **IPv6 지원 (PR #205)** — 🔄 **PARTIAL (Stages 1+2)**: PVA TCP/UDP 서버 IPv6 동작. **Stage 1**: `PvaServerConfig::bind_ip: Ipv4Addr` → `IpAddr` 일반화 (`crates/epics-pva-rs/src/server_native/runtime.rs:35`), `[::1]:5075`/`[::]:5075` TCP bind 가능; `PvaServer::client_config()`는 server bind family를 따라 v4/v6 loopback 자동 선택; pva-gateway-rs/dual-gateway-rs binaries `--bind ::1` 수용. **Stage 2**: 신규 `run_udp_responder_v6` (`udp.rs`) — `enable_ipv6_udp = true`일 때 `PvaServer::start`가 `[::]:udp_port` 별도 listener task 추가 spawn, v6 SEARCH 수신 후 동일 GUID/TCP-port로 SEARCH_RESPONSE. v4 path와 parallel 운영 (AsyncUdpV4 미수정 — non-invasive). PVA wire는 이미 16-byte IPv4-mapped IPv6 인코딩(`proto/ip.rs::ip_to_bytes`). Linux 기본 `IPV6_V6ONLY=0` 자동 dual-stack, BSD/macOS는 v6-only. 3 integration tests (v6 TCP bind+dial, v6 end-to-end pvget round-trip, v6 SEARCH→SEARCH_RESPONSE wire-level). **남은 작업**: PVA client의 IPv6 multicast SEARCH (FF0E::400) + beacon emit, `EPICS_PVA_ADDR_LIST` IPv6 v6-preference 정책. CA는 wire format 4-byte IP 제약으로 IPv4 유지(upstream PR #205도 EPICS_HAS_IPV6=1 컴파일 플래그 DRAFT 상태).
