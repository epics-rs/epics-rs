...
| [#831](https://github.com/epics-base/epics-base/pull/831) | 2026-03 | caRepeater run-time debug switch | `crates/epics-ca-rs/src/repeater/` | done — `run_repeater_with_debug(level)` + `ca-repeater-rs -d`/`-dd` clap arg (count). Level 1 prints client lifecycle ("New client", "Verified N", "refused", "Deleted"); level 2 adds per-message "Sent to port N" / "Client on port N is alive". Output to stderr; `-d` implicitly keeps stderr open (no detach_stdio). |
| [#788](https://github.com/epics-base/epics-base/pull/788) | 2026-02 | Avoid overreporting available CPUs (cgroup/affinity) | tokio runtime worker thread sizing in `epics-rs` runtime entry | done — Rust `std::thread::available_parallelism()` (1.66+) honours `sched_getaffinity` + cgroup limits. Used by `iocsh/commands.rs:829` (iocStats) and `ad-plugins-rs/par_util.rs`. |
| [#742](https://github.com/epics-base/epics-base/pull/742) | 2025-11 | `aao`, `aai` VAL field type `DOUBLE[]` → `FTVL[]` | aao/aai record VAL polymorphism in `crates/epics-base-rs/src/server/records/` | done — `WaveformRecord` (shared by aai/aao/subArray) already polymorphic on FTVL via `reallocate_val()` — VAL is `EpicsValue::{Char,Short,Long,Float,Double}Array` per FTVL. menuFtype CHAR/SHORT/LONG/FLOAT/DOUBLE handled. PR #742 was the C end-of-evolution toward this design; Rust port started here. |
...
| [#711](https://github.com/epics-base/epics-base/pull/711) | 2025-10 | Allow CA clients to determine server protocol version | `crates/epics-ca-rs/src/client/` server-version exposure | done — `transport.rs` captures `server_minor_version` on CA_PROTO_VERSION receipt and dispatches ECHO (v4.3+) vs READ_SYNC accordingly. |
| [#708](https://github.com/epics-base/epics-base/pull/708) | 2025-10 | Expand dbEvent synchronization | dbEvent path in `crates/epics-base-rs/src/server/db/event*` | N/A — epics-rs has no dbEvent worker thread / extra-labor queue (replaced by `RecordInstance::notify_*` direct mpsc fan-out from caller task context). `db_flush_extra_labor_event()` etc. have no equivalent target. `testMonitorSync` semantics emerge naturally because notify is synchronous w.r.t. the writer's lock release. |
| [#689](https://github.com/epics-base/epics-base/pull/689) | 2025-10 | Better guesses for wrong field names | dbAccess parser error path | done — `dbpf` typo suggestion implemented in iocsh round 23. |
...
| [#678](https://github.com/epics-base/epics-base/pull/678) | 2025-11 | hex/octal strings in dbPut/dbGet | dbPut/dbGet string-to-numeric parser | done — `parse_string_to_f64` (commit 17210b4) handles sign + 0x hex + 0-leading octal + decimal/exponent. Integer types and Double/Float share the same parser. |
...
| [#636](https://github.com/epics-base/epics-base/pull/636) | 2025-06 | `EPICS_DB_INCLUDE_PATH` for dbLoadTemplate | dbLoadTemplate in epics-base-rs | done — `iocsh/commands.rs:876` reads the env var and constructs the include search path for db loading. |
...
| [#608](https://github.com/epics-base/epics-base/pull/608) | 2025-10 | Warn to stderr when discarding CPP modifier for outlink | dblink/outlink parser | done — `record_instance.rs::put_common_field("OUT", _)` checks for `" CP"` / `" CPP"` suffix and emits `tracing::warn!` naming record + field, mirroring C `dbParseLink`'s `errlogPrintf`. |
...
| [#359](https://github.com/epics-base/epics-base/pull/359) | 2026-02 | Fix undefined timestamp for NORD field | aao/aai/waveform NORD timestamp path | done — structurally enforced: every notify path (`process_record_with_links_inner`, `AsyncPendingNotify`, `complete_async_record_inner`) calls `apply_timestamp` before snapshot/`notify_from_snapshot`, so NORD events always carry the post-process timestamp. Regression test `test_array_records_nord_monitor_uses_post_process_timestamp` covers all 4 ArrayKind. |
...
| [#862](https://github.com/epics-base/epics-base/pull/862), [#863](https://github.com/epics-base/epics-base/issues/863) | DNS TTL refresh of HAG / CA hostname state | `/reload-acf` introspection exists; no automatic TTL refresh | done — `EPICS_RS_HAG_DNS_REFRESH_SECS=N` env var spawns a periodic task in `CaServer::run` that re-reads the registered ACF source path and re-resolves every HAG hostname → IP set. Default 0 (disabled) keeps historic on-demand-only behaviour. Implementation factored to `Self::reload_acf_inner(&path, &acf, &reload_tx)` so the timer task uses cloned handles without holding the full `&self` borrow. |
| [#803](https://github.com/epics-base/epics-base/pull/803) | TOUT common record field | not in base either; track | watch |
| [#796](https://github.com/epics-base/epics-base/pull/796) | msi post-expansion calc syntax | msi-equivalent in epics-rs | N/A — msi (Macro Substitution and Include) is a separate C tool; epics-rs handles macros via `db_loader::substitute_macros` which runs at .db parse time only. No post-expansion `calc` syntax in our pipeline; sites needing computed substitution use templating outside the IOC. |
| [#776](https://github.com/epics-base/epics-base/pull/776) | Soft Time Part device support | timestamp dtyp split | DRAFT-blocked — upstream PR not merged; INP parser format / device support API undecided. |
| [#775](https://github.com/epics-base/epics-base/pull/775) | bi/bo conversion improvements | `records/{bi,bo}.rs` | DRAFT-blocked — upstream Issue OPEN (label `question`); proposed bi/bo `ZRVL`/`ONVL` + redefined MASK has no agreed wire/field layout. |
| [#673](https://github.com/epics-base/epics-base/pull/673) | iocsh readline ctrl+C close stdin | iocsh signal handling | DRAFT-blocked — upstream PR DRAFT. Current rustyline behaviour (Ctrl+C cancels line, Ctrl+D EOF) matches operator expectations. |
| [#671](https://github.com/epics-base/epics-base/pull/671) | atExit on SIGTERM/SIGINT | epics-rs shutdown signal handling | done — `IocApplication::run` `tokio::select!` polls `signal::ctrl_c` and `signal::unix::SignalKind::terminate`; signal arrival drops the runner future, RAII cleans up spawned tasks. |
| [#629](https://github.com/epics-base/epics-base/pull/629) | caget INT→SHORT in dbr | CA wire DBR mapping | N/A — `caget-rs`'s `-d/--dbr-type` is parity-only (unwired); when wired, the same `INT`→`SHORT` mapping will fold in trivially. |
| [#621](https://github.com/epics-base/epics-base/pull/621) | nameserver: force CA protocol version | `EPICS_CA_NAME_SERVERS` handshake version selection | DRAFT-blocked — upstream PR DRAFT (review comments). Defer transport.rs version override until upstream picks env var name / compile flag. |
...
| [#503](https://github.com/epics-base/epics-base/pull/503) | caget tolerates partial disconnect | `caget` tool behavior | done — `caget-rs` (`bin/caget-rs.rs`) spawns one task per PV, collects results in parallel, and prints success/failure independently. Disconnect on one PV ("not connected" / "timeout") doesn't abort siblings; the program exits non-zero only after iterating all results. |
...
| [#475](https://github.com/epics-base/epics-base/pull/475) | asLib type-safety incompat | ACF library | N/A — C asLib's type-safety patch tightened void-pointer casts in ACF callbacks. epics-rs `epics_base_rs::server::access_control` parses ACF into typed Rust structs (`AccessConfig`, `Hag`, `Asg`, `Rule`) — no void-pointer surface; type-mismatch is caught at compile time. |
...
| [#154](https://github.com/epics-base/epics-base/pull/154) | DBR_VFIELD virtual field | CA DBR types | deferred — upstream DRAFT (DBR_VFIELD never reached master). Adding a new DBR class requires CA wire-protocol negotiation + new class enum + per-record virtual-field handlers. Defer until upstream design stabilises. |
| [#149](https://github.com/epics-base/epics-base/pull/149) | Address modifiers | dblink modifiers | done — link parser at `record/link.rs:285` supports MS/MSS/MSI/NMS/NPP/CP/CPP/PP modifier suffixes, applied via `MonitorSwitch` + `LinkProcessPolicy`. |
| [#69](https://github.com/epics-base/epics-base/pull/69) | Specified TCP port + UDP 5064 fixed | server port env vars (decouple TCP/UDP) | done — commit `9d8a34b`: `cas_server_port()` env helper, `CaServerBuilder::tcp_port`, `IocApplication::tcp_port`. UDP responder advertises actual bound TCP port in SEARCH_REPLY. |
...
| [#558](https://github.com/epics-base/epics-base/pull/558) (already in B) | 2025-06 | `afterIocRunning` iocsh command | iocsh commands | done — `IocApplication::queue_after_ioc_running` + iocsh `cmd_after_ioc_running`. `IocApplication::run` Phase 2e drains queue on a fresh `IocShell` after PINI/device/autosave. |
...
| [#213](https://github.com/epics-base/epics-base/pull/213) | 2022-01 | Hex literals in hardware links (e.g. `@dev 0xFF`) | `link.rs::ParsedLink::Hw(HwLink { kind, args, raw })` with `try_parse_hw_link` — `@dev arg…` (InstIo) + `#Cn Sn` (VmeIo); hex literals preserved as full-token args. 4 tests | **done (commit 4745366)** |
...
| [#86](https://github.com/epics-base/epics-base/pull/86) | 2021-01 | Add JSON5 support (trailing commas, hex, comments in db files) | `link.rs::try_parse_json_link` recognizes `{const: …}`, `{ca: { pv: "…" }}`, `{pva: { pv: "…" }}` (unquoted-key + single-quote tolerant). JSON5 leniency on inline db files (trailing commas / hex / comments inside `{}`) is the remaining surface; the JSON-link grammar itself is in. 7 tests | **done (commit f21410f); JSON5 leniency partial** |
