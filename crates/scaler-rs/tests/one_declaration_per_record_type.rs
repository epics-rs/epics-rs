...
//! wrong: `OUT` was declared writable where
//! `scalerRecord.dbd` says `special(SPC_NOMOD)`, and not one of the 341 fields
//! carried the `menu()` its declaration names — the choice strings a client
//! sees came from a second hand-written table (`menu_field_choices`) instead.
//!
...
use scaler_rs::records::dbd_generated::{RECORD_TYPES, record_fields};
...
fn records() -> Vec<Box<dyn Record>> {
    vec![Box::new(scaler_rs::records::scaler::ScalerRecord::default())]
}
