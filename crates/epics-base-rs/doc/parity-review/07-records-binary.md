...
- **HISTORY (superseded by upstream `c9817fa59` / PR #817):** this finding once claimed `biRecord.c` runs an AFTC low-pass filter, was then "corrected" to say `biRecord.c` carries **no** `AFTC`/`AFVL` at all (true only up to the 2009 Codeathon filter `824d37811`, which covered `ai`/`calc`/`longin`/`mbbi` + later `int64in`), and the port's earlier bi-AFTC field was removed on that basis. **Both are now stale:** upstream `c9817fa59` (PR #817) ADDED the AFTC alarm-range filter to `biRecord`, so modern `biRecord.c` DOES carry `AFTC`/`AFVL`. The six C AFTC records are now `ai`/`calc`/`longin`/`int64in`/`mbbi`/`bi`. The port re-added them to bi (merge `21d006ce`), matching `biRecord.c` exactly — including the UDF-path subtlety that leaves `AFVL` untouched (unlike `ai`/`mbbi`, which zero it). The STATE/COS gap flagged above remains separately fixed via the `Record::check_alarms` hook.
