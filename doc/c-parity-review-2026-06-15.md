...
Severity: Medium — feature (assessment needed)
...
C: `NDFileHDF5` passes `compressionAware=true` (`NDFileHDF5.cpp:2268`) and implements direct-chunk-write: `configureCompression` reads `pArray->codec` and overrides the HDF5 filter settings from the pre-compressed array (`NDFileHDF5.cpp:3314-3331` — BLOSC/BSLZ4/LZ4/JPEG → matching `HDF5Compress*` type), so the compressed bytes are written through the corresponding HDF5 filter without re-compression.
Impact: With a pre-compressed input array, C writes a chunked dataset carrying the compressed bytes under the matching HDF5 filter (the areaDetector direct-chunk-write optimisation); the Rust writer cannot reproduce this. Reachability/closure depends on whether rust-hdf5 0.2.17 exposes a direct-chunk-write API (`H5DOwrite_chunk` analog) and an externally-defined-filter pipeline — verify the backend before attempting. Until then this is the HDF5 sibling of [ADP-98]; the `compression_aware()` override must NOT be applied to the Rust HDF5 writer standalone, because the consume path does not exist.
